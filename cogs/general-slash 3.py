...
class General(commands.Cog, name="general-slash"):
    ...
    async def botinfo(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description="Sirius official server",
            color=0x9C84EF
        )
        embed.set_author(
    ...
    async def server(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        """
        embed = disnake.Embed(
            description=f"Join the support server for the bot by clicking [here](https://discord.gg/Jw9hswWYZW).",
            color=0xD75BF4
        )
        try:
            await interaction.author.send(embed=embed)
    ...
    @checks.not_blacklisted()
    async def ethereum(self, interaction: ApplicationCommandInteraction) -> None:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=EUR") as request:
                if request.status == 200:
                    data = await request.json(
                        content_type="application/json")  # For some reason the returned content is of type JavaScript
                    embed = disnake.Embed(
                        title="Ethereum price",
                        description=f"The current price is {data} 🪙",
                        color=0x9C84EF
                    )
                else:
                    embed = disnake.Embed(
    ...
    async def bitcoin(self, interaction: ApplicationCommandInteraction) -> None:
        ...
        # This will prevent your bot from stopping everything when doing a web request - see: https://discordpy.readthedocs.io/en/stable/faq.html#how-do-i-make-a-web-request
        async with aiohttp.ClientSession() as session:
            async with session.get("https://min-api.cryptocompare.com/data/price?fsym=BTC&tsyms=EUR") as request:
                if request.status == 200:
                    data = await request.json(
                        content_type="application/json")  # For some reason the returned content is of type JavaScript
                    embed = disnake.Embed(
